...
describe("enrichDataSource tests", () => {
  ...
  test(
    "enrichDataSource enriches records and updates data_record.json and columnDefs",
    async () => {
      ...
      expect(recordAfterRemoval?.json).not.toHaveProperty(
      ...
      expect(
        dsAfterRemoval?.columnDefs.find(
          (c) => c.name === enrichmentColumnName,
        ),
      ).toBeUndefined();
...
describe("enrichDataRecords tests", () => {
  ...
  test(
    "enrichDataRecords enriches dirty records and updates data_record.json and columnDefs",
    async () => {
      // 1. Create test organisation
      const org = await upsertOrganisation({
