...
interface DataSourceSelectionModalProps {
  ...
  selectedMemberDataSourceId: string | null;
  selectedMarkerDataSourceIds: string[];
  onSelectMemberDataSource: (dataSourceId: string | null) => void;
  onSelectMarkerDataSource: (dataSourceId: string, selected: boolean) => void;
}

export default function DataSourceSelectionModal({
  open,
  onOpenChange,
  dataSources,
  selectedMemberDataSourceId,
  selectedMarkerDataSourceIds,
  onSelectMemberDataSource,
  ...
  const groupedOtherDataSources = useMemo(() => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-none max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose which data sources to add to this map
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 flex flex-col gap-6">
          ...
          {memberDataSources && memberDataSources.length > 0 && (
            <div>
              ...
              <div className="divide-y rounded-md border border-neutral-200 bg-white">
                {memberDataSources.map((dataSource) => {
                  const isSelected =
                    dataSource.id === selectedMemberDataSourceId;
                  return (
                    <button
                      key={dataSource.id}
                      onClick={() =>
                        onSelectMemberDataSource(
                          isSelected ? null : dataSource.id,
                        )
                      }
                      className={cn(
          ...
          {otherDataSources && otherDataSources.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-sm font-semibold flex items-center gap-2">
                  <MarkerCollectionIcon color="#6b7280" />
              ...
              {groupedOtherDataSources.length === 0 ? (
                <div className="text-center py-8 text-neutral-400 border rounded-md">
                  ...
                  <p className="text-sm">No other data sources yet</p>
                </div>
              ) : (
                    ...
                    <div key={group.recordType}>
                      ...
                      <div className="divide-y rounded-md border border-neutral-200 bg-white">
                        {group.items.map((dataSource) => {
                          const isSelected =
                            selectedMarkerDataSourceIds.includes(dataSource.id);
                          return (
                            <button
                              key={dataSource.id}
                              onClick={() =>
