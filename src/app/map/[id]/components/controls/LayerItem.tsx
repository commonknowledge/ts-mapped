...
export default function LayerItem({
    ...
    const { getDataSourceVisibility, setDataSourceVisibilityState } = useContext(MarkerAndTurfContext);
    ...
    const color = layerType === 'members' ? mapColors.member.color : layerType === 'locations' ? mapColors.markers.color : mapColors.areas.color;
    ...
    const handleVisibilityToggle = (e: React.MouseEvent) => {
        ...
        if (onVisibilityToggle) {
            ...
            setDataSourceVisibilityState(dataSourceId, !getDataSourceVisibility(dataSourceId));
            ...
            console.warn('Individual visibility enabled but no onVisibilityToggle handler provided');
        } else {
    ...
    return (
        ...
        >
            {showEyeIcon && (
                ...
                >
                    <Eye className={`w-4 h-4 ${visibilityState ? 'text-neutral-500' : 'text-neutral-400'}`} />
                </button>
            )}
            <div className="w-1 self-stretch mr-2 rounded" style={{ backgroundColor: color }} />
