...
function findActiveSection<T extends { active?: boolean }>(
...
export default async function SupportPage() {
  ...
  const featuresSection = findActiveSection<FeaturesSection>(
    support.featuresSection,
  );
  const whySupportSection = findActiveSection<WhySupportSection>(
    support.whySupportSections,
  );
  const whatSupportersGetSection = findActiveSection<WhatSupportersGetSection>(
    support.whatSupportersGetSections,
  );
  const whoThisIsForSection = findActiveSection<WhoThisIsForSection>(
    support.whoThisIsForSections,
  );
    ...
    <>
      ...
      {statsSection && (
        <div className="bg-brand-background">
          <Container>
            <div
              className={`flex flex-col ${statsSection.image ? "lg:flex-row" : ""} items-center gap-8 md:gap-12 py-16 md:py-20`}
            >
              ...
              <div className="flex-1 w-full text-center">
                ...
                {statsSection.stats && statsSection.stats.length > 0 && (
                  <div
                    className={`mt-8 md:mt-12 flex flex-col ${statsSection.stats.length === 3 ? "md:grid md:grid-cols-3" : statsSection.stats.length === 2 ? "md:grid md:grid-cols-2" : ""} gap-6 md:gap-8 max-w-4xl mx-auto`}
                  >
                ...
                {videoSection.videoPlaybackId ? (
                  ...
                  <div className="relative w-full aspect-video bg-neutral-200 flex items-center justify-center">
                    <div className="text-center p-8">
                      ...
                      <TypographyP className="text-neutral-500">
                        Video placeholder
                      </TypographyP>
          ...
          <Container>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {featuresSection.cards.map((card, index) => (
                  ...
                  >
                    ...
                    >
                      {renderIcon(
                        (card.iconType as "people" | "impact" | "action") ||
                          "impact",
            ...
            <div className="text-center mt-8">
              <TypographyP className="text-lg font-medium text-neutral-800 italic">
                This is not {'"'}nice to have{'"'} — it{"'"}s movement-critical
                infrastructure.
              </TypographyP>
            </div>
          </Container>
          ...
          <Container>
            <div className="max-w-3xl mx-auto">
              ...
              {whySupportSection.description && (
                <TypographyP className="text-lg text-neutral-700">
                  {whySupportSection.description}
                </TypographyP>
              )}
            </div>
      ...
      {support.imageBreak && (
        <div className="w-full object-cover h-96">
          <Image
            src={urlFor(support.imageBreak).url()}
            alt={support.title}
            width={1000}
            height={1000}
            className="w-full h-full object-cover"
      ...
      {whatSupportersGetSection && (
        <div className="bg-neutral-50 py-16 md:py-20">
          <Container>
            ...
            {whatSupportersGetSection.benefits &&
              whatSupportersGetSection.benefits.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <Container>
            <div className="max-w-3xl mx-auto">
              ...
              {whoThisIsForSection.description && (
                <TypographyP className="text-lg text-neutral-700 mb-6">
                  {whoThisIsForSection.description}
                </TypographyP>
              )}
              {whoThisIsForSection.audiences &&
                whoThisIsForSection.audiences.length > 0 && (
                  <ul className="space-y-3">
                    {whoThisIsForSection.audiences.map((audience, index) => (
                      <li key={index} className="flex items-start">
                        <span className="mr-3 mt-1 text-brand-blue">•</span>
                        <TypographyP className="text-neutral-700">
                          {audience}
                        </TypographyP>
                      </li>
                    ))}
